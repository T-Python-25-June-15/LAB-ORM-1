...
def createBlogs_view(request:HttpRequest):
    ...
    if request.method =="POST":
        ...
        new_blog=Blog(title=request.POST["title"],content=request.POST["content"],is_published = "is_published" in request.POST, published_at=published_at, image= request.FILES["image"])
        new_blog.save()
